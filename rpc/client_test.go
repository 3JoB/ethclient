...
func TestClientRequest(t *testing.T) {
	...
	var resp echoResult
	if err := client.Call(&resp, "test_echo", "hello", 10, &echoArgs{S: "world"}); err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(resp, echoResult{String: "hello", Int: 10, Args: &echoArgs{S: "world"}}) {
...
func TestClientResponseType(t *testing.T) {
	...
	if err := client.Call(nil, "test_echo", "hello", 10, &echoArgs{S: "world"}); err != nil {
	...
	err := client.Call(resultVar, "test_echo", "hello", 10, &echoArgs{S: "world"})
	if err == nil {
...
func TestClientBatchRequest(t *testing.T) {
	...
	batch := []BatchElem{
		{
			Method: "test_echo",
			Args:   []any{"hello", 10, &echoArgs{S: "world"}},
			Result: new(echoResult),
		},
		{
			Method: "test_echo",
			Args:   []any{"hello2", 11, &echoArgs{S: "world"}},
			Result: new(echoResult),
		},
		{
	...
	wantResult := []BatchElem{
		{
			Method: "test_echo",
			Args:   []any{"hello", 10, &echoArgs{S: "world"}},
			Result: &echoResult{String: "hello", Int: 10, Args: &echoArgs{S: "world"}},
		},
		{
			Method: "test_echo",
			Args:   []any{"hello2", 11, &echoArgs{S: "world"}},
			Result: &echoResult{String: "hello2", Int: 11, Args: &echoArgs{S: "world"}},
		},
		{
			Method: "no_such_method",
			Args:   []any{1, 2, 3},
...
func TestClientNotify(t *testing.T) {
	...
	if err := client.Notify(context.Background(), "test_echo", "hello", 10, &echoArgs{S: "world"}); err != nil {
		t.Fatal(err)
	}
}
...
func TestClientHTTP(t *testing.T) {
	...
	var (
		...
		wantResult = echoResult{String: "a", Int: 1, Args: new(echoArgs)}
	)
	defer client.Close()
	for i := range results {
		i := i
