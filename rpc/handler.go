...
func (b *batchCallBuffer) timeout(ctx context.Context, conn jsonWriter) {
	...
	for _, msg := range b.calls {
		if !msg.isNotification() {
			resp := msg.errorResponse(&internalServerError{code: errcodeTimeout, message: errMsgTimeout})
...
func (h *handler) handleBatch(msgs []*jsonrpcMessage) {
	...
	if len(msgs) == 0 {
		h.startCallProc(func(cp *callProc) {
			resp := errorMessage(&invalidRequestError{message: "empty batch"})
...
func (h *handler) handleImmediate(msg *jsonrpcMessage) bool {
	...
	case msg.isResponse():
		...
		h.log.Trace("Handled RPC response", "reqid", idForLog{RawMessage: msg.ID}, "duration", time.Since(start))
		return true
	default:
		return false
	}
...
func (h *handler) handleResponse(msg *jsonrpcMessage) {
	...
	if op == nil {
		h.log.Debug("Unsolicited RPC response", "reqid", idForLog{RawMessage: msg.ID})
		return
	}
...
func (h *handler) handleCallMsg(ctx *callProc, msg *jsonrpcMessage) *jsonrpcMessage {
	...
	case msg.isCall():
		...
		var ctx []any
		ctx = append(ctx, "reqid", idForLog{RawMessage: msg.ID}, "duration", time.Since(start))
		if resp.Error != nil {
		...
		return resp
	case msg.hasValidID():
		return msg.errorResponse(&invalidRequestError{message: "invalid request"})
	default:
		return errorMessage(&invalidRequestError{message: "invalid request"})
	}
}
...
func (h *handler) handleCall(cp *callProc, msg *jsonrpcMessage) *jsonrpcMessage {
	...
	if err != nil {
		return msg.errorResponse(&invalidParamsError{message: err.Error()})
	}
	start := time.Now()
...
func (h *handler) handleSubscribe(cp *callProc, msg *jsonrpcMessage) *jsonrpcMessage {
	...
	if callb == nil {
		return msg.errorResponse(&subscriptionNotFoundError{namespace: namespace, subscription: name})
	}
